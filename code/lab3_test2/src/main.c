...
void thread_3(void) {
  ...
  while(1) {
    print_status("3", cnt++);
    spin_wait(30-PRINT_STATUS_TIME_MS);
    wait_until_next_period();
  }
}
...
int main(void) {
  ...
  status = thread_create(&thread_1, &thread1_stack[USR_STACK_WORDS-1], 1, 10, 1000);
  if (status) {
    printf("Failed to create thread 1: %d\n", status);
    return 1;
  }

  status = thread_create(&thread_2, &thread2_stack[USR_STACK_WORDS-1], 2, 20, 2000);
  if (status) {
    printf("Failed to create thread 2: %d\n", status);
    return 1;
  }

  status = thread_create(&thread_3, &thread3_stack[USR_STACK_WORDS-1], 3, 30, 3000);
  if (status) {
