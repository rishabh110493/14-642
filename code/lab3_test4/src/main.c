 ...
 * @file   main.c
 *
 * @brief  Test for rate-monotonic scheduling, with enforcement
 */
...
/** @brief Thread which exceeds its allotted execution time
 */
void thread_2(void) {
  int cnt = 0;
  while(1) {
    print_status("2", cnt++);
    spin_wait(500);
    // IMPORTANT! Does not call wait_until_next_period
  }
}
