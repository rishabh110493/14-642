...
int main(void) {
  ...
  status = thread_create(&thread_1, &thread1_stack[USR_STACK_WORDS-1], 1, 200, 500);
  if (status) {
    printf("Failed to create thread 1: %d\n", status);
    return 1;
  }

  status = thread_create(&thread_2, &thread2_stack[USR_STACK_WORDS-1], 2, 300, 1000);
  if (status) {
    printf("Failed to create thread 2: %d\n", status);
    return 1;
  }

  status = thread_create(&thread_3, &thread3_stack[USR_STACK_WORDS-1], 3, 300, 1500);
  if (status) {
