...
static long velocity_buffer[VELOCITY_SAMPLES];
...
/**
* @brief Averages the speed over 100 samples of speed
*		and returns it.
* 		   
*/  
...
/**
* @brief Examines the Gray code read from the encoder
		to sense the direction in which the encoder was turned.
* 		   
*/  
int getDirection(int prevA,int prevB, int A, int B )
{
 ...
 /**
 * @brief IRQ Handler for Encoder 0. Called everytime the value
 * 		  of encoder 0 changes.
 * 		   
 */   
static irq_handler_t enc0_irq_handler(unsigned int irq, void *dev_id, struct pt_regs *regs)
{
 ...
 /**
 * @brief IRQ Handler for Encoder 1. Called everytime the value
 * 		  of encoder 1 changes.
 * 		   
 */  
