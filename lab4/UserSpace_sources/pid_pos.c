...
#define ANGLE_LIMIT_MID 		180
#define SPEED_LIMIT				100
#define SPEED_LIMIT_LOW_THRES	16
#define SPEED_LIMIT_HI_THRES	50
...
int main(){
   ...
   if(init_file())
	...
	while(1)
	{
		//Treating -x degrees as 360-x degrees.
	   target_pos = getEncoderParameter(fd_encoder,ENCODER_ANGLE);
	   target_pos = (target_pos<0)?(target_pos+ANGLE_LIMIT):target_pos;
	   
	   motor_pos_read = getEncoderParameter(fd_encoder,MOTOR_ANGLE);
	   motor_pos_read = (motor_pos_read<0)?(motor_pos_read+ANGLE_LIMIT):motor_pos_read;
	   ...
	   error_pos = target_pos - motor_pos_read;
		...
		if(error_pos < 0)
		{
			...
			else
			{
				...
				error_pos = -ANGLE_LIMIT - error_pos;
			}
		}
	   ...
	   //Calculating the actuation velocity for the motor
		motor_speed_write =  (int)(Kp*error_pos) - (int)(Kd*(error_pos_prev - error_pos)) + (int)(Ki*error_sum);
		error_pos_prev = error_pos;
		error_sum += error_pos;
	   
		//Making sure the actuation of the motor happens within reasonable limits
		if(motor_speed_write<0)
		   motor_speed_write  = motor_speed_write *-1;		
		if(motor_speed_write>SPEED_LIMIT_HI_THRES)
		   motor_speed_write  = SPEED_LIMIT_HI_THRES;	  	   
		if((motor_speed_write<SPEED_LIMIT_LOW_THRES) && (motor_speed_write >0))
		   motor_speed_write = SPEED_LIMIT_LOW_THRES;
