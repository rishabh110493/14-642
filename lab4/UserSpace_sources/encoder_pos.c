...
int main(){
   ...
   if(init_file())
	...
	while(1)
	{
	   ...
	   encoder_pos = (encoder_pos<0)?(encoder_pos+ANGLE_LIMIT):encoder_pos;
	   motor_angle = getEncoderParameter(fd_encoder, MOTOR_ANGLE);
	   motor_angle = (motor_angle<0)?(motor_angle+ANGLE_LIMIT):motor_angle;
	   
	   error_angle = encoder_pos - motor_angle;

		if(error_angle < 0)
		{
			if(error_angle>-ANGLE_LIMIT_MID)
			{
				direction = CLOCKWISE;
			}
			else
			{
				...
				error_angle = -ANGLE_LIMIT - error_angle;
			}			
		}
		else
		{
